...
| **🔗 Topics link to each other (Phase 10)** | ✅ **LINK-01…11 done, 2026-09-19.** Tumo chose **automatic name matching**. Two tiers, and they are different types: a curated link carries a required reason; a `Mention` has no `why` field at all, so an invented reason is not representable (`SP-090`). **87 topics · 26 derived mentions · 9 curated links**, and both of Tumo's examples work in both directions. **Waiting on Tumo:** `article` has no route (modal or a 25th route?) · thematic candidates for a ruling per SP-030, including the parked `module:vilakazi → vilakazi-street` · **the curated tier and the person pages have not been seen in a browser** — the dev server's renderer froze; verified by test and data only. The real lever on coverage is **PAGE-06**: 26 links is what 1–3 sentences per place buys. · decisions: [docs/sim_plan.md §4.12](docs/sim_plan.md) |
