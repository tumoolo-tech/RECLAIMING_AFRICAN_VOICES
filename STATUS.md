...
_Last updated: 2026-09-23 (hotfix) — by Furn (via Claude). Previous update: 2026-09-23 (#38)._
_Last updated: 2026-09-23 (#40) — by Furn (via Claude). Previous update: 2026-09-19 (#51)._
...
| **🧾 Audit backlog — [issue #58](https://github.com/tumoolo-tech/RECLAIMING_AFRICAN_VOICES/issues/58)** (40 issues, ordered; **South Africa first**, continental group parked 18 Sep) | 🟡 **#36 #25 #34 #51 merged · #40 in review (#77) · #38 done (this branch)** — the reviewer kit: `npm run review:sheet`, harm-tiered (tier 1 = 34 strings), `reviewedUi` + named reviewers, [reviewer guide](specs/reviewer-guide.md) · **needs Tumo: is Setswana content actually reviewed?** (gold tick vs. the DRAFT notes in the content files) · week-1 next: #45 #43 #35 |
| **🔁 PR checks (CI)**: GitHub Actions runs typecheck + unit tests + docs lint + **`build:web`** on every PR into `main` ([.github/workflows/pr-checks.yml](.github/workflows/pr-checks.yml)); Vercel still deploys | 🟡 **#40 done (this branch)** — `scripts/run-ts.mjs` makes the 11 TypeScript-loading scripts work on any Node 22.6+ (the flag is passed only where it is needed, never on 26+); the build is now a CI step · **still not a required check** — that plus `engines`/`.nvmrc` awaits Tumo |
