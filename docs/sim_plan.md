...
| **T4** | 2026-09-17 | Every place carries its own `sources`. **No source → not published** | [AGENTS.md §4](../AGENTS.md). A tourism product that invents heritage is worse than one that omits it | Locked |
...
| ID | Decision | Why | Status |
|---|---|---|---|
| **SP-035** | **Place detail is an overlay inside `CityScreen` — not a 25th `Route` member** | Two grounded reasons. [App.tsx:106](../app/App.tsx) records that inlining a stage in the route switch "made the type-checker recurse over the (now 24-member) route union until it stopped finishing." And [`ArticleReader`](../app/src/components/ArticleReader.tsx) is *already* a full-screen modal mounted by `NationalDaysScreen`, not a route — the precedent exists. This also avoids editing `Route`, `ATLAS_ROOMS` and `KEYED_ROUTES` | Proposed |
| **SP-036** | **Consequence, recorded rather than discovered later:** SP-035 means **no deep link or shareable URL to a place in v1** | An honest cost. If the Department wants a linkable place page, that reopens SP-035 with the tsc risk priced in | Proposed |
...
| **SP-067** | **No coordinate can be sourced to SP-054 standard.** None of the four institutions publish decimal coordinates — museums publish street addresses. The candidates are Wikipedia, which SP-054 does not admit | Found doing the research, not anticipated when SP-014 chose to fill `coords`. By our own rule (SP-052: a coordinate is a factual claim) these cannot ship as they stand. **(a)** Tumo verifies each against an official map at the published street address, cited as such · **(b)** amend SP-054 to admit Wikipedia for locational facts only · **(c)** reverse SP-014 and drop `coords`. **Blocks TOUR-05b entirely.** Default if unanswered: **(a)** — keeps SP-054 intact, ~10 minutes of work | **Open** |
| **SP-068** | **A street has no coordinate.** `vilakazi-street` can only carry a *representative* point, not a fact about the street | The same shape problem that made a bike tour an `Experience` rather than a `Place` of `kind: "route"`, arriving now for a `street`. Either use the Mandela House corner and say so in `sources`, or make `coords` optional after all — which partly reopens SP-014 | **Open** — blocks `vilakazi-street` only |
...
[AGENTS.md §4](../AGENTS.md) exists to prevent, arrived at through layout rather than prose, but
