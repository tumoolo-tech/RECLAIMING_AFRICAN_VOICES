...
_Last updated: 2026-09-24 (Botlhale ready for Setswana) — via Claude. Previous update: 2026-09-24 (story book narrated)._
...
| **🔁 PR checks (CI)**: typecheck + tests + docs lint + `build:web` on every PR into `main` ([.github/workflows/pr-checks.yml](.github/workflows/pr-checks.yml)) | 🔴 **NOT a required check — and on 23 Sep that cost us a broken `main`.** The #77/#78 `package.json` conflict was resolved keeping both sides, producing invalid JSON; CI went red on the merge commit and the merge landed anyway. **Tumo: make this a required status check in the repo settings** — no code needed, highest-value setting in the repo · also still open and also Tumo's: `engines` in `app/package.json` and an `.nvmrc` (neither exists yet, so nothing pins the Node version that #77's launcher was written for) |
| **🧾 Audit backlog — [issue #58](https://github.com/tumoolo-tech/RECLAIMING_AFRICAN_VOICES/issues/58)** (40 issues, ordered; **South Africa first**, continental group parked 18 Sep) | 🟡 **#36 · #25 · #34 · #51 · #40 · #38 all merged** (PRs #69, #71, #72, #75, #77, #78) — log split into STATUS-LOG.md (union-merged), product name and stack claims corrected, `npm run check:docs` in CI; `scripts/run-ts.mjs` makes the 11 TypeScript-loading scripts work on any Node 22.6+ and `build:web` is now a CI step; the reviewer kit — `npm run review:sheet`, harm-tiered (tier 1 = 34 strings), `reviewedUi` + named reviewers, [reviewer guide](specs/reviewer-guide.md) · **needs Tumo: is Setswana content actually reviewed?** (gold tick vs. the DRAFT notes in the content files) · week-1 next: #45 #43 #35 |
...
| **🔗 Topics link to each other (Phase 10)** | ✅ **LINK-01…11 done, 2026-09-19.** Tumo chose **automatic name matching**. Two tiers, and they are different types: a curated link carries a required reason; a `Mention` has no `why` field at all, so an invented reason is not representable (`SP-090`). **87 topics · 26 derived mentions · 9 curated links**, and both of Tumo's examples work in both directions. **Waiting on Tumo:** `article` has no route (modal or a 25th route?) · thematic candidates for a ruling per SP-030, including the parked `module:vilakazi → vilakazi-street` · **seen working on both a place page and a person page**; the one unseen change is the heading fix that seeing it prompted. The real lever on coverage is **PAGE-06**: 26 links is what 1–3 sentences per place buys. · decisions: [docs/sim_plan.md §4.12](docs/sim_plan.md) |
| **🎞️ A story told by scrolling (Phase 11)** | ✅ **STORY-01…06 done, 2026-09-19.** Tumo asked for the Rockstar GTA VI shape. **"Sixteen June"** — nine panels, four licensed place photographs, Sam Nzima's archival photograph (`SP-102`, Tumo's rights call), four typographic beats, sources at the end. Own route from Home; nothing existing changed. A panel never carries its own image path (`SP-100`); a documentary photograph is shown whole and undarkened rather than cropped under a headline (`SP-101`). **Motion fixed + deepened 2026-09-21:** the animation was **not running at all** — `story` was missing from `OWN_SCROLL`, so the shell scrolled the route and the story's own `onScroll` never fired (`SP-105`; measured, not guessed). Now also **reduced-motion aware** (`SP-106`) · eased reveals · per-line stagger · no entrance for panels already read (`SP-107`). **Two readings, 2026-09-21** (`SP-108`): **Scroll** (places go past you) and **In the place** — a held shot per place, photograph pinned for ~2.5 screens while the story arrives on it a line at a time, chosen on the title card, session-only (no store, no PII). Nzima's photograph is **never staged** — the mode stops performing and falls back to the whole, undarkened record (`SP-109`). Reduced motion collapses the stages entirely (10083px → 4449px, nothing hidden). **Opens as a book, 2026-09-24** (`SP-114`): the default reading is now **Book** — the same paper spread and page turn as the literary Reader (shared `Book.tsx`), title → nine spreads → sources; **In the place** and **Scroll** switch from the top bar. **Narrated, 2026-09-24** (`SP-115`): the book has a language picker and **Listen** — machine drafts of the story in all ten other languages, labelled unreviewed (LANG-13), read aloud through the existing ladder (ElevenLabs en/af, Botlhale when keyed, device voice with an honest note). **Botlhale made ready for a real account** (`SP-116`): refresh-token exchange, audio cached, siSwati/isiNdebele skip it — **needs Tumo: a Botlhale refresh token in `app/.env`** (there is no `app/.env` at all yet, so every language is on the device voice, English included). **Open:** STORY-07, more stories — blocked on prose, like everything else. · decisions: [docs/sim_plan.md §4.13](docs/sim_plan.md) |
